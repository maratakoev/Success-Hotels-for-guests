    ...
    return GestureDetector(
      onTap: () async {
        if (widget.service.options != null &&
          ...
          // Если у услуги есть опции, показываем диалог
          showDialog(
            context: context,
            builder: (context) {
          ...
          // Если опций нет, отправляем только id
          try {
            // Создаем объект ServiceRequest
            final serviceRequest = ServiceRequest(
              responseServiceId: widget.id, // Передаем только ID услуги
              responseOptions: null, // Опции отсутствуют
            );
            ...
            if (response.data['success']) {
              // Открываем страницу оплаты, если получили confirmationUrl
              ...
              if (confirmationUrl != null) {
                ...
                // Выводим сообщение об ошибке, если confirmationUrl равен null
                _showErrorMessage('Ошибка: confirmation_url равен null');
              }
            } else {
              // Выводим сообщение об ошибке, если success == false
              _showErrorMessage(
